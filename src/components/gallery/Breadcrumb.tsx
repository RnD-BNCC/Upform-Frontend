...
export type FolderView =
  ...
  | { level: "respondent"; eventId: string; eventName: string; responseId: string; respondentLabel: string };
...
export function Breadcrumb({
  ...
  if (view.level === "event" || view.level === "respondent") {
    segments.push({
      ...
      view: { level: "event", eventId: view.eventId, eventName: view.eventName },
    });
  }
      ...
      {segments.map((seg, i) => {
        ...
        return (
          <span key={i} className="flex items-center gap-1">
            {i > 0 && <CaretRight size={13} weight="bold" className="text-gray-400" />}
            <button
