...
import { PAGE_TYPE_ICON_CLASS, PAGE_TYPE_ICONS } from "@/constants";
import type { FormSection } from "@/types/form";
import type { BuilderPageType } from "@/types/builder";

type AddPageType = BuilderPageType;
...
function getPageTabTitle(page: FormSection) {
  const pageType = (page.pageType ?? "page") as AddPageType;
...
function TabContextMenu({
  ...
  const pageName = getPageTabTitle(page);
...
export default function PageTabBar({
  ...
  const pageRenderKeys = useMemo(() => {
    const seen = new Map<string, number>();

    return pages.map((page, idx) => {
      const baseKey = page.id?.trim() || `page-${idx}`;
      ...
      <div className="h-12 bg-white flex items-center px-3 gap-2 shrink-0 z-20">
            ...
            {addMenuOpen && (
              ...
              >
                ...
                {ADD_PAGE_OPTIONS.map((opt) => (
                  ...
                  >
                    <span
                      className={`mt-0.5 ${PAGE_TYPE_ICON_CLASS[opt.type]}`}
                    >
        ...
        >
          {pages.map((page, idx) => {
            ...
            const pageRenderKey = pageRenderKeys[idx];

            return (
              <motion.div
                key={pageRenderKey}
                layout
                layoutId={pageRenderKey}
              ...
              >
                <span
                  className={`${PAGE_TYPE_ICON_CLASS[pageType]} shrink-0`}
                >
                ...
                {isEditing ? (
                  ...
                  <span>{getPageTabTitle(page)}</span>
                )}
